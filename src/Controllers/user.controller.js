...
const loginUser = asyncHandler(async (req,res) => {
    ...
    // if (!(username || email)) {                                       //we need any one of them
...
const updateAccountDetails = asyncHandler(async(req, res) => {
    ...
    const user = await User.findByIdAndUpdate(
        req.user?._id,
        {
            $set: {
                fullName,
                email,           //fullName:fullName, email: email  (can be done like this)
            }
        },
        {new: true}
    ...
    const user = await User.aggregate([
        ...
        },
        {
            $lookup: {
                ...
                as: "watchHistory",
                pipeline: [
                    {                                                //sub pipe line for owner access(user) - 1
                        $lookup: {
                            from: "users",
                            localField: "owner",
                            foreignField: "_id",
                            as: "owner",
                            pipeline: [                     //further sub pipe line for to remove extra info of user -1
                                {
                                    $project: {
                                        fullName: 1,
                                        username: 1,
                    ...
                    {                                                //sub pipe line for owner access(user) - 2
                        $addFields:{
                            owner:{
                                $first: "$owner"
                            }
